...
import psutil  # get logical cpu count with command: psutil.cpu_count()
...
def x_generate_surface_alpha_array(im, sl):
    ...
    # generates intensity slope angles (in radians) for each pixel, for intensity profile along x axis
...
def y_generate_surface_alpha_array(im, sl):
    ...
    # generates intensity slope angles (in radians) for each pixel, for intensity profile along y axis
    # im = image as numpy array
    # sl = slope correction factor
...
def generate_imgGradients (im, **kwargs):
    ...
    # generate arrays of surface slope angles (in radians) for each pixel
...
def generate_x_peakArray_5 (im, **kwargs):
    # 08/17/24 scans horizontal line profiles for peaks (i.e. along x axis)
    # output is 2-tuple with y (rows) and x (columns) coords (format similar to output of np.where)
    ...
    promMin = kwargs.get('promMin', 5)  # the smaller, the higher sensitivity and noise !
    promMax = kwargs.get('promMax', 255)
    widthMin = kwargs.get('widthMin', 3)  # take into account expansion factor (assumed default is 5, works with 3 too !)
    widthMax = kwargs.get('widthMax', 25)  # take into account expansion factor (assumed default is 5, works with 3 too !)
...
def neuriteSegmentation_X_4 (imE, **kwargs):
    ...
    #
    # input: image 'imE' np.uint8, assumed to be expanded
    #     'imE' will not be expanded or rotated inside this function
    # Function scans for peaks along horizontal (x-axis) profile lines, segments peaks as neurites or non-neurits
    ...
    def pixel_dirArr_2(p, **kwargs):
        ...
        # Any version of pixel_dirArr_x must be paired with same version of isNeurite_x
        ...
        # cv2.imshow('scT', resizeImg_nearest(scT, 10))

        mask = np.ones((hF, wF), dtype=np.uint8) * 255
        # mask[:, [0, 1, 9, 10, 11, 19, 20]] = 0
        # mask[:,[0,10,20]] = 0
        mask[:,[10]] = 0 # new 09/06/24, relaxes filter considerably (ca. 60% of peaks pass before isNeurite)

        scT_filt = np.bitwise_and(scT, mask)
        # cv2.imshow('sct_filt', resizeImg_nearest(scT_filt, 10))

        xAr = np.array([[-1, -1]] * hF, dtype=int)
        dirAr = np.array([], dtype=float)


        # NOTE 09/06/24 this should be changed in all versions !
        # countL = 0
        # countR = 0

        filled_L = [] # replaces countL
        filled_R = [] # replaces countR
        ...
        # if (countL >= hF - missing) & (countR >= hF - missing):  # SETTING SETTING SETTING !!!
    ...
    def pixel_dirArr_4(p, **kwargs):
        ...
        missing = kwargs.get('missing', 4)

        hH = 6 # h for height, H for half edge # changed (was 3)
        wH = 10
        ...
        return dirAr
        # NOTE 08/30/24 dirAr may contain -1
        # dirAr is array with dirAr.shape = (7,2), contains vector projection directions left and right of pixel p

    # end of: pixel_dirArr_4()
    ...
    def isNeurite_4(dirAr):
        ...
        if len(dirAr) > 0:
            ...
            # avdif_arL = np.average(dif_arL)
            # avdif_arRr = np.average(dif_arL)

            # Crit_11 and Crit_12 both don't discriminate at all, strangely (supposed to capture slighly curved neurites)
            # Crit_11 = (0.05 < avdif_arL < 0.2) and (0.05 < avdif_arRr < 0.2)
            # Crit_12 = (-0.2 < avdif_arL < -0.05) and (-0.2 < avdif_arRr < -0.05)
            ...
            # # Cin_4 (average) is same as Cin_5 with 50% percentile
            # Cin_4 = np.average(abs(np.subtract(arL_, arRr_))) < 0.5
            # Cin_5 - best criterion ! (as of 09/06/24)
            #     Cin_5 tests how parallel vectors in same row in arL_ and arRr_ are
            ...
            # Cex_5 excludes points surrounded by radially arranged vectors (as typical for blebs)
            Cex_5 = (np.average(dif_arL) > 0.05) & (np.average(dif_arRr) < -0.05) # NOTE 09/05/24 this was 0.02
            # NOTE 09/07/24 Cex_5 of questionable value - filter function may overlap with Cin_5 (both test parallelism)
            # > 0.1  -> 56.28 / 39.18
            # > 0.05 -> 52.46 / 33.82
            # > 0.02 -> 47.67 / 30.04
            # > 0.01 -> 45.76 / 28.99
            ...
            # Cex_5 (centrifugality) may not be useful, somehow redundant with Cin_5
...
# imageCorrection_new_3(im, lim_I_new = 180, perc_hi = 90, gamma = 0.7, promMin = 5, mx = 5, ord = 2)
def imageCorrection_new_3(im, **kwargs):
    # 09/14/24 modification - includes sgolay2d noise removal step
    # 09/04/24
    # NOTE 09/05/24 consider:
    # Amplification of signal intnsity seems to rather decrease perc_av, maybe lim_I_new should be 150 to 200
    ...
    lim_I_new = kwargs.get('lim_I_new', 180)
    perc_hi = kwargs.get('perc_hi', 90)
    gamma = kwargs.get('gamma', 0.7)
    promMin = kwargs.get('promMin', 5)
    mx = kwargs.get('mx', 5)
    ord = kwargs.get('ord', 2)

    # assumption: im in original size
    # peaks along horizontal (x) scan lines

    pkTu_x, _ = generate_x_peakArray_5(im, promMin=promMin, promMax=255, widthMin=1, widthMax = 6, distMin = 1, heightRel = 0.1)
    # peaks along vertical (y) scan lines
    ...
    if len(pkL) == 0:
        ...
        im_out = im.astype(np.float64)
        im_out = im_out * lim_I_new/pkI_perc_hi
        im_out = 255 * np.power((im_out / 255), gamma)

        # MODIFICATION 09/14/24
        im_out = sgolay2d(im_out, mx, ord, None)
        # end of: MODIFICATION 09/14/24

        im_out = np.rint(np.clip(im_out, 0, 255)).astype(np.uint8)
        # cv2.imshow('im_out_after', im_out)

    return im_out, ex_sta
# end of: imageCorrection_new_3(im, **kwargs)
...
def nX_segmentation (im, im_name, **kwargs):
    ...
    for C in [C0, C30, C60, C90, C120, C150]:
        if C[2] > 0:
            perc_list.append(100 * C[3]/C[2])
        else:
            perc_list.append(-1)

    perc_list_nonzero = [p for p in perc_list if p > -1]
    if len(perc_list_nonzero) > 0:
        perc_av = np.average(np.array(perc_list_nonzero))
    else:
        perc_av = -1

    D = {'img_file': im_name, 'C0': C0, 'C30': C30, 'C60': C60, 'C90': C90, 'C120': C120, 'C150': C150, 'perc': perc_list, 'perc_av': perc_av}

    # What the four numbers in C0 mean (same for C30, C60, etc.):
    ...
    # perc_list with six numbers lists Cx[3]/Cx[2] for C0, C30, C60, etc.
    # perc_av is average of perc_list
...
def nX_segmentation_test (im, im_name, **kwargs):
    ...
    D = {'img_file': im_name, 'C0': C0, 'C30': C30, 'C60': C60, 'C90': C90, 'C120': C120, 'C150': C120, 'perc': perc_list, 'perc_av': perc_av}
...
# CHANGE REQUIRED ! replace <path_main> with absolute path for folder with repository files
# path_main = <path_main>
path_main = 'C:/Users/Paul/Paul Box XPS 17/Special/Python/220227_NDI/241019_segmentation_NII_MASTER/'
os.chdir(path_main)
from neuriteX_methods import * # neuriteX_methods must be located in content root folder


img_files = sorted([os.path.basename(x) for x in glob.glob('{}*.png'.format('./img_ori/'))])

# read image
imgX = cv2.imread('./img_ori/' + img_files[0], -1)
...
D, stack = nX_segmentation_test (imgC, img_fname, ptUL=(10,10), eH=100, extF = 3, win = 3, ord=2, t = 100000)
imwrite(path_sim + 'newstack_' + img_fname, stack, photometric='minisblack')  # imwrite from tifffile

# image segmentation
D = nX_segmentation (imgC, img_files[0], ptUL=(10,10), eH=500, extF=3, win=3, ord=2, t=100000)

# end of: single image analysis
##########################################

# <<<<<<<<<<<<<<<<<<<<<<<<<<<<
# IMAGE CORRECTION - local batch processing

from neuriteX_methods import * # neuriteX_methods must be located in content root folder

# CHANGE THIS ! replace <path_main> with absolute path for root folder
# path_main = <path_main>
path_main = 'C:/Users/Paul/Paul Box XPS 17/Special/Python/220227_NDI/241019_segmentation_NII_MASTER/'
os.chdir(path_main)
...
# end of: IMAGE CORRECTION - local batch processing
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
# IMAGE SEGMENTATION - local batch processing
# processing time ~25 min for eH=200
...
def seg_parallel(im_fname):
    ...
    D = nX_segmentation(im, im_fname, ptUL=(400,400), eH=200, extF=3, win=3, ord=2, t=100000)
    return D
...
# end of: IMAGE SEGMENTATION - local batch processing
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
df_R1 = df_R1.loc[:, ['file','img_file', 'well', 'time_n', 'state', 'perc_av', 'condition']]
...
# group by well, state, scene - then calculate sum of perc_av over all time points, and store in new variable 'sum_ref'

df_R2['sum_ref'] = df_R2.groupby(['well', 'state', 'scene'])['perc_av'].transform('sum')

# calculate mean of 'sum_ref' for each well and state
df_R2['sum_ref_mean'] = df_R2.groupby(['well', 'state'])['sum_ref'].transform('mean')
# correct perc_av values
df_R2['perc_av_corr'] = df_R2.perc_av * df_R2.sum_ref_mean/df_R2.sum_ref
# df_R2['perc_av_corr_add'] = df_R2.perc_av + df_R2.sum_ref_mean - df_R2.sum_ref
...
for well in df_R3.well:
    ...
    w_ref_cut = np.average(df_R3.loc[b0 & b1 & b_cut, 'perc_av_corr'])
    w_ref_uncut = np.average(df_R3.loc[b0 & b1 & b_uncut, 'perc_av_corr'])

    df_R3.loc[b0 & b_cut, 'NII_well'] = df_R3.loc[b0 & b_cut, 'perc_av_corr']/w_ref_cut
    df_R3.loc[b0 & b_uncut, 'NII_well'] = df_R3.loc[b0 & b_uncut, 'perc_av_corr']/w_ref_uncut
